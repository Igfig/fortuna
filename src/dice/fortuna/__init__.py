...
class Fortuna(object):
	...
	def __init__(self, config, botClass=DefaultBot):
		# parse config
		self.name = config['BOT CONFIG']['nickname']
		self.banter = BanterController(config['BOT CONFIG']['banter_file'], self.name)
		
		self.parsers = []
		for pname in set([p.strip() for p in config['BOT CONFIG']['parser'].split(',')]):
			try:
	...
	def handle_msg(self, msg):
		...
		if exceptions and not parsed:
			...
			return ["".join([random.choice("bfghkl") for _ in range(random.randint(9, 12))])]
