...
TODO: the code for putting parens around a roll if it's inside another dice expression is really ugly; 
		it uses isinstance(), which is bad. Can we figure out another way to do it? 
TODO: don't super like the output format for things like 2d(1d6)

TODO: dist situations have a problem still: the end comment needs to be put on the multiroll, 
		and the prev comments need to go on the end
		current situation: 
	...
	def __new__(cls, die, status=0):
		...
		try:
			...
			try:
				...
				val = random.randint(1, int(die.size))	
											
			except AttributeError:
				#"die" is actually a DiceResult or RollResult
				val = random.randint(1, int(roll_if_dice(die.size)))
...
#		return self.roll()
	...
	def __repr__(self):
		...
		if isinstance(self.num, Dice) or isinstance(self.num, Roll):
			full_string += "(" + str(self.num) + ")"
		else:
			full_string += str(self.num)
...
class DiceResult(list):
	...
	def __init__(self, dice, comment="", sort=False, status=0):
		...
		rolls = [die.roll(status) for _ in range(int(num))]
...
class RollResult: #TODO: add some more sequence emulation methods
