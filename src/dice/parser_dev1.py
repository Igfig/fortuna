...
TODO: maybe we shouldn't get rid of parens if they're being used solely for math and 
	not for nesting rolls
		i.e. 2 + (3 - 5) vs (2d4)d6
		Oddly we do do this for actual rolls, sorta...
		(1d4*2) - 3 -> ([1] * 2 = 2) - 3 = -1
TODO: any line starting with a number will be treated as a die roll.
	maybe have something like, if the output string would be the same as the input, 
	don't return it?
	well no, because that'd kill 1 + 1 = 2
	see, the problem we're getting is 1 fart -> 1 fart = 1 

FIXME: 1d4#1d4 is a crash, and it has to do with  [""] * RollResult. (also one time 
	1d4x1d4 did something similar)
	basically it doesn't know how to multiply those together properly? It tries to 
	cast the result as a list or whatever
...
from dice.rollable import Dice, DiceResult, DiceInt, Roll, RollResult, \
		COMPARISON_FUNCTIONS, roll_if_dice
...
operators = "\+|\*{1,2}|-|/|\^|%"
signifiers = ",|;|:|{|}|\(|\)"
commentender = "(?:" + operators + "|" + signifiers + ")\s*[\d(]" 
bracestr = "{([\w[\]]*)}" # why the square brackets inside? Oh it's for the weird implementation I have of referencing the dice from the final pattern, I think
numstr = "-?\s*(?:{[\w[\]]*}|\d+)" # FIXME don't permit arbitrary strings inside braces.

dice_manip_str = "(?:(?P<hold>kh|kl|dh|dl|rh|rl)|(?P<advmanip>!!|r|rr)" + \
					"(?P<advcond>" + comparators + ")?)(?P<manipnum>" + numstr + \
					")|(?P<explode>!)"
commentstr = "(?P<comment>.*?)(?=(?:" + commentender + ")|$)"
...
parens_str = "\((?P<contents>[^()]*)\)" # XXX we shouldn't need this should we?
multistr = "(?:(?P<multi>.*?)x(?=\s*(?:-|\d|\(|\{)))?(?P<rolls>.*)"
fullstr = "(?:(?P<numlines>[^#]+)\s*#)?(?P<multis>[^#:]+)(?::(?P<distrib>.*))?"

bracepat = re.compile(bracestr)
dice_or_int_pat = re.compile(dicestr + "?" + commentstr)# i.e. everything but the
dice_pat = re.compile(dicestr + commentstr)				# first number is optional
dicepat_without_groups = re.compile(dicestr_without_groups)
parens_pat = re.compile(parens_str) # FIXME parens_pat converts parens to braces for 
multipat = re.compile(multistr)		# some reason later, which is bad with brace_pat
...
class DiceParser(object):
	...
	def parse_multiroll(to_parse, subrolls):
		...
		for multi_string in multi_strings:
			...
			multi = multimatch.group("multi") 
			if multi:
				#we do in fact have a roll x roll situation, so do the first one  
				x_roll = DiceParser.parse_roll(multi, subrolls)
				x_roll_result = roll_if_dice(x_roll)
				repetitions = int(x_roll_result)
			
			rolls = multimatch.group("rolls")
			multi_roll = DiceParser.parse_roll(rolls, subrolls)
...
def run_test_cases():
	test_cases = [
		...
		"1d4 horses.",
		"3d4 sheep, shaved",
		"3d3 cats, ocelots, and assorted felines",
		"2d8 billy-goats + 1d2 nannys",
		"2d6 dogs (on hats) + 3 cows",
		"1d6 + 3 scarfs/towels/butt-holsters",
		...
		"1d4 pants x 3d6", #dunno if I want to permit this?
		...
		"1d20+1 attack, 1d8+1 damage vs: bill, tim", #not quite right
		"1d6 + 1 hats for: john, dave + 2d4 socks", #no idea how this should even go
