...
TODO: fix import collisions
	I really want this to be a package called dice.fortuna.irc instead of a
	file called ircbot.py (and another called irclogger.py).
	but unfortunately the irc package collides with that. Any time I try to
	import from irc, it comes from dice.fortuna.irc instead of irc.
"""
import irc.bot, irc.client
import queue, random, threading, copy, re, configparser
import dice.starwars
from dice.fortuna import Message, Fortuna
from dice.fortuna.irclogger import Logger
...
class FortunaBot(irc.bot.SingleServerIRCBot):
	...
	def get_command_lines(self, msg):
		try:
			command, args = COMMAND_PAT.match(msg.line).groups()
		except AttributeError:
			# no match was found, so there's no command here
			return []
		...
		if command == "NOTE":
			...
			return self.logger.recall(msg.context, distance);
